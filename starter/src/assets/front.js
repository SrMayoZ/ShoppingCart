...
function handlePayment(amount) {
	...
	// Run the pay() function to get the remaining balance
	let cashReturn = pay(amount);

	// If the remaining balance is greater than or equal to zero, show a success message
	if (cashReturn >= 0) {
		div.innerHTML = `
		...
		// Otherwise, show a message indicating that more funds are needed
		div.innerHTML = `
        <p>Cash Received: ${currencySymbol}${amount}</p>
	...
	// Append the message to the payment summary div
...
function displayProducts(products) {
	...
	products.forEach((product) => {
        ...
        <div class="product">
          ...
          <button class="add-to-cart" data-productid="${product.productId}">Add to Cart</button>
        </div>
      `;
	});
